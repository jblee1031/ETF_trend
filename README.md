...
두 탭은 구성종목만 다르고 화면과 계산 로직은 같습니다. 각 종목의 5·10·20·60·120일 이동평균선을 계산해 다섯 개 섹션으로 보여줍니다.
...
5. **시장 요약** — 기준일 데이터로 매일 자동 생성. 아래 참조
...
기준일을 바꾸면 그날 기준으로 다시 만들어집니다. 수집 스크립트가 지수 시세와 **정배열이 아닌 종목까지 포함한 구성종목 전체**의 일간 등락을 함께 저장하고, 화면이 이를 규칙에 따라 문장과 표로 정리합니다.

- **요약문** — 지수 등락, 구성종목의 상승·하락 폭(하락 종목이 65% 이상이면 "넓게 하락"), 정배열 종목 수 변화. 지수와 종목 중간값의 방향이 반대면 대형주 영향이라고 덧붙입니다.
- **지수** — 종가, 당일·5일·20일 등락, 60거래일 고점 대비, 5·20·60·120일선 대비 위치, 60일 중 하루 ±6% 넘게 움직인 날 수
- **정배열 종목 수 추이** — 최근 6거래일의 단기·완전·미네르비니 통과 수와 당일 상승·하락 종목 수
- **섹터 등락** — 시가총액 가중 강세·약세 상위 4개 (2종목 이상 섹터만)
- **등락률 상위·하위** 5종목
- **전일 대비 정배열 변화** — 단기·완전 정배열의 신규 진입과 이탈, 이탈 전 유지 일수
- **관찰 포인트** — 조건에 해당할 때만 표시: 완전 정배열 수가 최근 10일 최고치의 절반 이하로 줄었을 때, 완전 정배열 이탈 종목이 단기까지 한꺼번에 깨졌을 때, 10일 이상 유지하던 종목의 편출, 단기 정배열의 30% 이상이 한 섹터일 때, 최근 5일 사이 정배열이 늘어난 섹터가 당일 -3% 이상 떨어졌을 때, 지수가 60일 중 5일 이상 ±6% 넘게 움직였을 때

코스닥150 지수는 데이터 소스에서 받을 수 없어 **코스닥 종합지수**로 대체합니다. 섹터 등락은 최신 시가총액으로 가중하며 유동주식 비율은 반영하지 않습니다.

### 등락률 기준

테이블의 등락률과 시장 요약의 상승·하락·섹터 등락은 모두 **거래소 기준가 대비 공식 등락률**(네이버 일별 시세)을 씁니다. 기준가는 보통 전일 종가와 같지만 거래소가 따로 정하는 날이 있어서, 종가끼리 나누면 증권사 앱과 다른 숫자가 나올 수 있습니다. 실제로 2026-09-15에는 대부분 종목의 기준가가 9/14 종가와 달랐고, 종가 대비 종가로 계산하면 상한가 종목(삼현)이 +33.08%로 나와 코스닥 가격제한폭(±30%)을 넘었습니다. 공식 값으로는 +29.80%입니다. 일별 시세를 받지 못한 종목은 종가 대비 종가로 대신합니다. 이동평균과 정배열 판정은 이 차이와 무관하게 일별 종가로 계산합니다.
